...
CNST_PATH: Path = Path(__file__).parent / "py_nasa_api/client/const.py"
...
if not CNST_PATH.is_file():
    print(f"Invalid path for const.py: {CNST_PATH}", file=sys.stderr)
    sys.exit(1)

if not TOML_PATH.is_file():
    print(f"Invalid path for pyproject.toml: {TOML_PATH}", file=sys.stderr)
    sys.exit(1)

# Get file contents
init_text = INIT_PATH.read_text("utf-8", "ignore")
cnst_text = CNST_PATH.read_text("utf-8", "ignore")
toml_text = TOML_PATH.read_text("utf-8", "ignore")

# Extract the current version
init_version_match = VERSION_INIT_REGEX.search(init_text)
cnst_version_match = VERSION_CNST_REGEX.search(cnst_text)
...
cnst_version = cnst_version_match.group(2) if cnst_version_match is not None else None
toml_version = toml_version_match.group(2) if toml_version_match is not None else None


# Check for consistency
if init_version != cnst_version or init_version != toml_version:
    print(
        f"Conflicting versions found in __init__.py, const.py and pyproject.toml: "
        f"{init_version} | {cnst_version} | {toml_version}",
        file=sys.stderr,
    )
    sys.exit(1)
...
cnst_replacement = VERSION_CNST_REGEX.sub(_replacement_formatter, cnst_version_match.group(0))
...
CNST_PATH.write_text(cnst_text.replace(cnst_version_match.group(0), cnst_replacement), "utf-8", "ignore")
